...
int main() {
    ...
    std::cout << " Size of integer on this machine: " << sizeof(int)
              << " bytes\n";
    std::cout << " Size of streampos on this machine: "
              << sizeof(std::streampos) << " bytes\n";
        ...
        while (true) {
            ...
            if (first_packet) {
                ...
                if (size_of_int != sizeof(int) ||
                    size_of_spos != sizeof(std::streampos)) {
                    std::cout
                            << "  Warning! basic type size on remote machine does not match local size\n";
                    std::cout << "  Size of integer on remote machine: "
                              << size_of_int << " bytes\n";
                    std::cout << "  Size of streampos on remote machine: "
                              << size_of_spos << " bytes\n\n";
                    break;
                }
            } else {
                ...
                time_span = std::chrono::duration_cast<std::chrono::duration<double> >(
                        clock2 - clock1); // Time between packets in seconds
            ...
            if (total_size <= min_pack_size) {
                ...
                size_t fname_size = total_size - (2 + 4 * size_of_int) -
                                    size_of_spos; // Size of the filename (in bytes)
                char *fname = new char[fname_size + 1];

                unsigned int index = 2 + size_of_int;
                memcpy(fname, (char *) &buffer[index], fname_size);
                index += fname_size; // Copy the file name
                memcpy((char *) &new_size, (char *) &buffer[index],
                       size_of_spos);
                index += size_of_spos; // Copy the file size
                memcpy((char *) &spillID, (char *) &buffer[index], size_of_int);
                index += size_of_int; // Copy the spill ID
                memcpy((char *) &buffSize, (char *) &buffer[index],
                       size_of_int);
                index += size_of_int; // Copy the buffer size
                memcpy((char *) &end_packet_flag, (char *) &buffer[index],
                ...
                if (magnitude < 3) {
                    ...
                    std::cout << "  Total file size: " << new_size / 1E3
                              << " kB\n";
                } // kB
                else if (magnitude >= 6 && magnitude < 9) {
                    std::cout << "  Total file size: " << new_size / 1E6
                              << " MB\n";
                } // MB
                else {
                    std::cout << "  Total file size: " << new_size / 1E9
                              << " GB\n";
                } // GB
                ...
                if (!first_packet) {
                    ...
                    if (magnitude > -3) {
                        ...
                        std::cout << "  Time diff: " << dT / 1E-3 << " ms\n";
                    } // ms
                    else if (magnitude <= -6 && magnitude > -9) {
                        std::cout << "  Time diff: " << dT / 1E-6 << " us\n";
                    } // us
                    else {
                        std::cout << "  Time diff: " << dT / 1E-9 << " ns\n";
                    } // ns

                    magnitude = order(rate);
                    if (magnitude < 3) {
                        std::cout << "  Data rate: " << rate << " B/s\n";
                    } // B/s
                    else if (magnitude >= 3 && magnitude < 6) {
                        std::cout << "  Data rate: " << rate / 1E3 << " kB/s\n";
                    } // kB/s
                    else if (magnitude >= 6 && magnitude < 9) {
                        std::cout << "  Data rate: " << rate / 1E6 << " MB/s\n";
                    } // MB/s
                    else {
                        std::cout << "  Data rate: " << rate / 1E9 << " GB/s\n";
                    } // GB/s
