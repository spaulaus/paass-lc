...
bool is_hribf_buffer(const unsigned int &input_) {
    return (input_ == HEAD || input_ == DATA || input_ == SCAL ||
            input_ == DEAD || input_ == DIR || input_ == PAC ||
            input_ == ENDFILE);
}

/// Generic BufferType constructor.
BufferType::BufferType(unsigned int bufftype_, unsigned int buffsize_,
...
void PLD_header::SetStartDateTime() {
    ...
    set_char_array(std::string(date_holder), start_date,
                   24); // Strip the trailing newline character
}
...
void PLD_header::SetEndDateTime() {
    ...
    set_char_array(std::string(date_holder), end_date,
                   24); // Strip the trailing newline character
}
...
bool PLD_header::Write(std::ofstream *file_) {
    ...
    if (debug_mode) {
        std::cout << "debug: writing " << 100 + total_title_len
                  << " byte HEAD buffer\n";
    }
...
bool PLD_header::Read(std::ifstream *file_) {
    ...
    file_->read(format, 16);
    format[16] = '\0';
    file_->read(facility, 16);
    facility[16] = '\0';
    file_->read(start_date, 24);
    start_date[24] = '\0';
    file_->read(end_date, 24);
    end_date[24] = '\0';
    ...
    file_->read(run_title, len_of_title);
    run_title[len_of_title] = '\0';
    file_->read((char *) &end_buff_check, 4);

    if (end_buff_check != buffend) { // Buffer was not terminated properly
        if (debug_mode) {
            std::cout << "debug: buffer not terminated properly\n";
        }
        return false;
    }
...
bool PLD_data::Read(std::ifstream *file_, char *data_, unsigned int &nBytes,
    ...
    if (check_bufftype != bufftype) { // Not a valid DATA buffer
        ...
        if (debug_mode) {
            std::cout << "debug: read an extra " << countw
    ...
    if (debug_mode) {
        std::cout << "debug: reading spill of " << nBytes << " bytes\n";
    }
...
/// Default constructor.
DIR_buffer::DIR_buffer() : BufferType(DIR,
                                      NO_HEADER_SIZE) { // 0x20524944 "DIR "
    this->Reset();
}
...
bool DIR_buffer::Read(std::ifstream *file_) {
    ...
    if (check_bufftype != bufftype ||
        check_buffsize != buffsize) { // Not a valid DIR buffer
    ...
    file_->read((char *) &run_num, 4);
    file_->seekg((buffsize * 4 - 20),
                 file_->cur); // Skip the rest of the buffer
...
bool HEAD_buffer::Read(std::ifstream *file_) {
    ...
    file_->read((char *) &check_buffsize, 4);
    if (check_bufftype != bufftype ||
        check_buffsize != buffsize) { // Not a valid HEAD buffer
        if (debug_mode) { std::cout << "debug: not a valid HEAD buffer\n"; }
        file_->seekg(-8, file_->cur); // Rewind to the beginning of this buffer
        return false;
    }

    file_->read(facility, 8);
    facility[8] = '\0';
    file_->read(format, 8);
    format[8] = '\0';
    file_->read(type, 16);
    ...
    file_->read(date, 16);
    date[16] = '\0';
    file_->read(run_title, 80);
    run_title[80] = '\0';
    file_->read((char *) &run_num, 4);
    file_->seekg((ACTUAL_BUFF_SIZE * 4 - 140),
                 file_->cur); // Skip the rest of the buffer
...
/// Default constructor.
DATA_buffer::DATA_buffer() : BufferType(DATA,
                                        NO_HEADER_SIZE) { // 0x41544144 "DATA"
    bcount = 0;
    good_chunks = 0;
    missing_chunks = 0;
    buff_pos = 0;
...
bool EOF_buffer::Read(std::ifstream *file_) {
    ...
    file_->read((char *) &check_buffsize, 4);
    if (check_bufftype != bufftype ||
        check_buffsize != buffsize) { // Not a valid EOF buffer
...
int PollOutputFile::SendPacket(Client *cli_) {
    ...
    if (!output_file.is_open() || !output_file.good()) {
        ...
        unsigned int index = 0;
        memcpy(&packet[index], (char *) &size_of_int, 1);
        index += 1;
        memcpy(&packet[index], (char *) &size_of_spos, 1);
        index += 1;
        memcpy(&packet[index], (char *) &bytes, sizeof(int));
        index += sizeof(int);
        ...
        bytes = (2 + 4 * sizeof(int)) + sizeof(std::streampos) +
                current_full_filename.size(); // Total size of the packet (in bytes)
        packet = new char[bytes];
        ...
        memcpy(&packet[index], (char *) &bytes, sizeof(int));
        index += sizeof(int);
        memcpy(&packet[index], (char *) str,
               (unsigned int) current_full_filename.size());
        index += current_full_filename.size();
        memcpy(&packet[index], (char *) &file_size, sizeof(std::streampos));
        index += sizeof(std::streampos);
        memcpy(&packet[index], (char *) &number_spills, sizeof(int));
        index += sizeof(int);
        memcpy(&packet[index], (char *) &buff_size, sizeof(int));
        index += sizeof(int);
        memcpy(&packet[index], (char *) &end_packet, sizeof(int));
    }
...
std::string
PollOutputFile::GetNextFileName(unsigned int &run_num_, std::string prefix,
