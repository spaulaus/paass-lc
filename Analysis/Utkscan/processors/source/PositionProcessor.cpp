...
bool PositionProcessor::Init(RawEvent &rawev) {
    ...
    while (true) {
        ...
        in >> location;
        if (in.eof()) {
            // place this here so a trailing newline is okay in the scan file
            break;
        }
...
void PositionProcessor::DeclarePlots() {
    ...
    for (int i = 0; i < maxNumLocations; ++i) {
        ...
        for (int j = 1; j < numQdcs; ++j) {
            ...
            if (i == 0) {
                ...
                histo.DeclareHistogram2D(
                        DD_QDCN__QDCN_LOCX + QDC_JUMP * j + LOC_SUM,
                        qdcBins, qdcBins, str.str().c_str());
                str.str("");
                str << "ALL QDC " << j << " NORM T/B";
                histo.DeclareHistogram1D(
                        D_QDCNORMN_LOCX + QDC_JUMP * j + LOC_SUM,
                        normBins, str.str().c_str());
            }
        }
        ...
        histo.DeclareHistogram1D(D_QDCTOTNORM_LOCX + i, normBins,
                                 str.str().c_str());
        str.str("");
        ...
        histo.DeclareHistogram2D(DD_QDCR2__QDCR1_LOCX + i, SA, SA,
                                 str.str().c_str());
        str.str("");

        str << "QDCR2 vs QDCR3, loc " << i;
        histo.DeclareHistogram2D(DD_QDCR2__QDCR3_LOCX + i, SA, SA,
                                 str.str().c_str());
        str.str("");

        str << "QDCR2 vs QDCR4, loc " << i;
        histo.DeclareHistogram2D(DD_QDCR2__QDCR4_LOCX + i, SA, SA,
                                 str.str().c_str());
        str.str("");

        str << "QDC1R vs Pos, loc " << i;
        histo.DeclareHistogram2D(DD_QDC1R__POS_LOCX + i, SA, SA,
                                 str.str().c_str());
        str.str("");

        str << "QDC2R vs Pos, loc " << i;
        histo.DeclareHistogram2D(DD_QDC2R__POS_LOCX + i, SA, SA,
                                 str.str().c_str());
        str.str("");

        str << "QDC3R vs Pos, loc " << i;
        histo.DeclareHistogram2D(DD_QDC3R__POS_LOCX + i, SA, SA,
                                 str.str().c_str());
        str.str("");

        str << "QDC4R vs Pos, loc " << i;
        histo.DeclareHistogram2D(DD_QDC4R__POS_LOCX + i, SA, SA,
                                 str.str().c_str());
        str.str("");
    }
    ...
    histo.DeclareHistogram2D(DD_POSITION, locationBins, positionBins,
                             "Qdc Position", 1, "pos");
}
...
bool PositionProcessor::Process(RawEvent &event) {
    ...
    for (vector<ChanEvent *>::const_iterator it = allEvents.begin();
        ...
        const ChanEvent *top = FindMatchingEdge(sumchan, topEvents.begin(),
                                                topEvents.end());
        const ChanEvent *bottom = FindMatchingEdge(sumchan,
                                                   bottomEvents.begin(),
                                                   bottomEvents.end());
        ...
        if (FindMatchingEdge(sumchan, topEvents.rbegin(), topEvents.rend()) !=
            top) {
            using namespace dammIds::position;
            // [4] -> Multiple top
