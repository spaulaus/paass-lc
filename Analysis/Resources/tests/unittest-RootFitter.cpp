...
TEST_FIXTURE(RootFitter, TestRootFitter) {
    CHECK_THROW(CalculatePhase(empty_vector_double, fitting_parameters, max_pair, baseline_pair), range_error);

    SetQdc(waveform_qdc);
    CHECK_CLOSE(-0.581124, CalculatePhase(waveform, fitting_parameters, max_pair, baseline_pair), 1.);
}
