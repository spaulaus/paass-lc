...
TEST_FIXTURE(PolynomialCfd, TestPolynomialCfd) {
    ...
    //Check that we throw a range error when the max position is larger than the data we provided.
    CHECK_THROW(CalculatePhase(trace_sans_baseline, cfd_test_pars, make_pair(trace_sans_baseline.size() + 3, 100),
    ...
    // "acceptable" value.
    CHECK_CLOSE(73.9898, CalculatePhase(trace_sans_baseline, cfd_test_pars, extrapolated_maximum_pair, baseline_pair), 5);
}
